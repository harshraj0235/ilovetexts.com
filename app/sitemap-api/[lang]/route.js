...
function getAlternatesXml(path) {
  let xml = `  <xhtml:link rel="alternate" hreflang="x-default" href="${SITE.url}${path}" />\n`;
...
export async function GET(request, { params }) {
  ...
  // Category Pages
  CATEGORIES.forEach((cat) => addUrl(`/${cat.id}`, '0.9', 'weekly', TODAY));

  // Tool Pages
  allTools.forEach((tool) => addUrl(`/${tool.categoryId}/${tool.slug}`, '0.85', 'daily', TODAY));

  // Blog index
  addUrl('/blog', '0.7', 'weekly', TODAY);
