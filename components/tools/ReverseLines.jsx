...
export default function ReverseLines({ t, lang }) {
  ...
  const processedData = useMemo(() => {
    ...
    if (skipEmpty) {
      ...
      nonEmpty.reverse();
      let idx = 0;
      let result = lines.map(l => l.trim() === '' ? l : nonEmpty[idx++]);
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      const savedInput = localStorage.getItem('ilovetexts_tc_reverselines_input');
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      localStorage.setItem('ilovetexts_tc_reverselines_input', input);
    }
  }, [input]);
  ...
  const handleDownload = () => {
    ...
    link.setAttribute("download", "ilovetexts_reverselines.txt");
    ...
    <div className="tool-workspace">
      ...
      <div className="tc-controls" style={{ display: 'flex', gap: '16px', marginBottom: '16px', flexWrap: 'wrap' }}>
        <label style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer', fontSize: '0.9rem' }}>
          ...
          Keep empty lines in original position (only swap text lines)
        </label>
      </div>
