...
// Helper: get a flat list of all tools with category info
...
export function getTool(categoryId, toolSlug) {
...
export function getRelatedTools(categoryId, currentSlug, limit = 5) {
  const cat = CATEGORIES.find(c => c.id === categoryId);
  if (!cat) return [];
  return cat.tools
...
export function getCrossLinks(currentCategoryId, limit = 4) {
  const otherCats = CATEGORIES.filter(c => c.id !== currentCategoryId);
  const links = [];
  for (const cat of otherCats) {
    if (links.length >= limit) break;
    links.push({
      slug: cat.tools[0].slug,
      name: cat.tools[0].name,
      icon: cat.tools[0].icon,
      description: cat.tools[0].description,
      categoryId: cat.id,
    });
  }
  return links;
