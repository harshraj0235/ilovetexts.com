...
export function generateAlternates(lang, path) {
  ...
  const englishCanonical = buildCanonical('en', path);
...
export function generateItemListSchema(category) {
  return {
    ...
    itemListElement: category.tools.map((tool, i) => ({
      '@type': 'ListItem',
      position: i + 1,
      // @id makes each item a resolvable entity Google can link to Knowledge Graph
...
export function generateWebSiteSchema(t, lang = 'en') {
  return {
    ...
    // SearchAction enables Google Sitelinks Searchbox in SERPs.
    // The search is handled client-side in CommandCenter.jsx via the
    // q= parameter — the /tools page accepts ?q= for filtering.
    potentialAction: {
      '@type': 'SearchAction',
      target: {
        '@type': 'EntryPoint',
    ...
    speakable: {
      ...
      cssSelector: ['#tool-finder-title', '#tool-finder-title + p'],
    },
  };
}
