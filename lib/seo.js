// ═══════════════════════════════════════════════════════
// SEO Helper — Meta tags, Schema markup, OG generation
// Upgraded: Unique meta per tool, removed fake ratings,
// added ItemList, WebSite, Organization schemas
// Enhanced: speakable, UseAction, sameAs, dateModified
// ═══════════════════════════════════════════════════════

import { SITE } from './tools-config';
import { getToolSEO } from './tools-seo-data';
import { buildCanonical, LANG_CODES } from './i18n';
import { INDEXABLE_TOOL_LOCALES, PRIMARY_CONTENT_LOCALE, isIndexableToolLocale } from './search-indexing';

// Use a fixed build date to prevent hydration mismatches and schema spam
const BUILD_DATE = '2026-09-06';

function getLocale(lang) {
  const map = { en: 'en_US', hi: 'hi_IN', pt: 'pt_BR', es: 'es_ES', de: 'de_DE', id: 'id_ID' };
  return map[lang] || 'en_US';
}

export function generateAlternates(lang, path, options = {}) {
  const canonicalLocale = options.canonicalLocale || lang;
  const locales = options.locales || LANG_CODES;
  const canonical = buildCanonical(canonicalLocale, path);
  const englishCanonical = buildCanonical(PRIMARY_CONTENT_LOCALE, path);
  const languages = { 'x-default': englishCanonical };
  locales.forEach(code => {
    languages[code] = buildCanonical(code, path);
  });
  return { canonical, languages };
}

export function generateToolMeta(tool, category, lang = 'en') {
  const seoData = getToolSEO(tool.slug);

  // Use unique hand-crafted meta if available, otherwise generate
  const title = tool.content?.metaTitle 
    || seoData?.metaTitle
    || `${tool.name} — Fast, Free & Secure Online Tool`;
  
  const description = tool.content?.metaDescription
    || seoData?.metaDescription
    || `${tool.description} Use ${tool.name} online for free with clear results and no account required.`;

  const path = `/${category.id}/${tool.slug}`;
  const canIndex = isIndexableToolLocale(lang);

  return {
    title,
    description,
    openGraph: {
      title,
      description,
      url: buildCanonical(lang, path),
      siteName: SITE.name,
      type: 'website',
      locale: getLocale(lang),
      images: [`${SITE.url}/og-image.png`],
    },
    twitter: {
      card: 'summary_large_image',
      title,
      description,
      images: [`${SITE.url}/og-image.png`],
    },
    alternates: generateAlternates(lang, path, {
      canonicalLocale: PRIMARY_CONTENT_LOCALE,
      locales: INDEXABLE_TOOL_LOCALES,
    }),
    robots: {
      index: canIndex,
      follow: true,
      googleBot: {
        index: canIndex,
        follow: true,
        'max-snippet': -1,
        'max-image-preview': 'large',
      },
    },
  };
}

export function generateCategoryMeta(category, t = null, lang = 'en') {
  const freeToolsText = t?.home?.heroBadge ? t.home.heroBadge.replace('{count}+ ', '').trim() : 'Free Online Tools';
  const toolsText = t?.ui?.tools ? t.ui.tools.toLowerCase() : 'tools';

  const title = `${category.name} — ${category.tools.length} ${freeToolsText}`;
  const description = `${category.description} ${category.tools.length} ${category.name.toLowerCase()} ${toolsText}. ${t?.trust?.instantTitle || 'Instant results'}, ${t?.trust?.privateTitle || '100% private'}, ${t?.ui?.noSignup || 'no signup required'}. ${category.tools.slice(0, 5).map(t => t.name).join(', ')}.`;

  const path = `/${category.id}`;
  const canIndex = isIndexableToolLocale(lang);

  return {
    title,
    description,
    openGraph: {
      title,
      description,
      url: buildCanonical(lang, path),
      siteName: SITE.name,
      type: 'website',
      locale: getLocale(lang),
      images: [`${SITE.url}/og-image.png`],
    },
    twitter: {
      card: 'summary_large_image',
      title,
      description,
      images: [`${SITE.url}/og-image.png`],
    },
    alternates: generateAlternates(lang, path, {
      canonicalLocale: PRIMARY_CONTENT_LOCALE,
      locales: INDEXABLE_TOOL_LOCALES,
    }),
    robots: {
      index: canIndex,
      follow: true,
      googleBot: { index: canIndex, follow: true, 'max-snippet': -1, 'max-image-preview': 'large' },
    },
  };
}

export function generateFAQSchema(faqs) {
  return {
    '@context': 'https://schema.org',
    '@type': 'FAQPage',
    mainEntity: faqs.map(faq => ({
      '@type': 'Question',
      name: faq.question,
      acceptedAnswer: {
        '@type': 'Answer',
        text: faq.answer,
      },
    })),
  };
}

export function generateToolSchema(tool, category, t, lang = 'en') {
  const toolUrl = buildCanonical(lang, `/${category.id}/${tool.slug}`);
  return {
    '@context': 'https://schema.org',
    '@type': 'WebApplication',
    '@id': `${toolUrl}#webapp`,
    name: `${tool.name} — ${SITE.name}`,
    applicationCategory: 'UtilitiesApplication',
    operatingSystem: t.schema.operatingSystem,
    browserRequirements: t.schema.browserRequirements,
    url: toolUrl,
    description: tool.description,
    // datePublished uses the site launch date — do NOT use new Date() here
    // as that would tell Google every tool was just published today.
    datePublished: '2025-08-01',
    dateModified: tool.content?.updatedAt || BUILD_DATE,
    inLanguage: lang === 'en' ? 'en-US' : lang,
    offers: {
      '@type': 'Offer',
      price: '0',
      priceCurrency: 'USD',
      availability: 'https://schema.org/InStock',
    },
    featureList: [
      t.schema.feature1,
      t.schema.feature2,
      t.schema.feature3,
      t.schema.feature4,
      t.schema.feature5,
      t.schema.feature6,
    ],
    // aggregateRating deliberately omitted from static schema.
    // Real user ratings are collected client-side in RatingWidget.jsx
    // and injected into a separate JSON-LD script by RatingSchema.jsx.
    // This avoids Google's fake/synthetic rating penalty (March 2022 update).
    author: {
      '@type': 'Organization',
      name: SITE.name,
      url: SITE.url,
    },
    creator: {
      '@type': 'Person',
      name: 'Harsh Raj',
      url: 'https://www.linkedin.com/in/harshitpatel9/',
    },
    potentialAction: {
      '@type': 'UseAction',
      target: {
        '@type': 'EntryPoint',
        urlTemplate: toolUrl,
      },
      name: t.schema.useAction.replace('{toolName}', tool.name),
    },
  };
}

export function generateBreadcrumbSchema(items) {
  return {
    '@context': 'https://schema.org',
    '@type': 'BreadcrumbList',
    itemListElement: items.map((item, i) => ({
      '@type': 'ListItem',
      position: i + 1,
      name: item.name,
      item: item.url,
    })),
  };
}

export function generateHowToSchema(tool, steps, t) {
  return {
    '@context': 'https://schema.org',
    '@type': 'HowTo',
    name: t.schema.howToName.replace('{toolName}', tool.name),
    description: t.schema.howToDesc.replace('{toolName}', tool.name).replace('{domain}', SITE.domain),
    totalTime: 'PT1M',
    tool: {
      '@type': 'HowToTool',
      name: t.schema.howToTool,
    },
    step: steps.map((step, i) => ({
      '@type': 'HowToStep',
      position: i + 1,
      name: step.title,
      text: step.description,
    })),
  };
}

// ItemList schema for category pages.
export function generateItemListSchema(category) {
  return {
    '@context': 'https://schema.org',
    '@type': 'ItemList',
    '@id': `${SITE.url}/${category.id}#itemlist`,
    name: `${category.name} Tools`,
    description: category.description,
    numberOfItems: category.tools.length,
    itemListElement: category.tools.map((tool, i) => ({
      '@type': 'ListItem',
      position: i + 1,
      '@id': `${SITE.url}/${category.id}/${tool.slug}`,
      name: tool.name,
      url: `${SITE.url}/${category.id}/${tool.slug}`,
      description: tool.description,
    })),
  };
}

// WebSite schema establishes the primary site entity.
export function generateWebSiteSchema(t, lang = 'en') {
  return {
    '@context': 'https://schema.org',
    '@type': 'WebSite',
    '@id': `${SITE.url}/#website`,
    name: SITE.name,
    url: SITE.url,
    description: t?.siteDescription || SITE.description,
    inLanguage: getLocale(lang).replace('_', '-'),
  };
}

// Organization schema — establishes brand entity in Knowledge Graph
export function generateOrganizationSchema() {
  return {
    '@context': 'https://schema.org',
    '@type': 'Organization',
    '@id': `${SITE.url}/#organization`,
    name: SITE.name,
    url: SITE.url,
    // Logo must be a dedicated logo image, not the OG image
    // Google's guidelines: logo should be 112x112px minimum, square preferred
    logo: {
      '@type': 'ImageObject',
      url: `${SITE.url}/favicon.ico`,
      width: 48,
      height: 48,
    },
    description: SITE.description,
    // foundingDate: use the actual launch year, not a placeholder
    foundingDate: '2025',
    founder: {
      '@type': 'Person',
      name: 'Harsh Raj',
      url: 'https://www.linkedin.com/in/harshitpatel9/',
      sameAs: [
        'https://www.linkedin.com/in/harshitpatel9/',
        'https://github.com/harshraj0235',
      ],
    },
    // sameAs helps Google connect your brand across platforms (Knowledge Graph)
    sameAs: [
      'https://github.com/harshraj0235/ilovetexts.com',
      'https://www.linkedin.com/in/harshitpatel9/',
      'https://www.producthunt.com/products/ilovetexts',
    ],
    contactPoint: {
      '@type': 'ContactPoint',
      contactType: 'customer support',
      url: `${SITE.url}/contact`,
      email: 'Contact@ilovetexts.com',
      availableLanguage: ['English', 'Hindi', 'Spanish', 'Portuguese', 'German', 'Indonesian'],
    },
  };
}
